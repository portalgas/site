...
abstract class modLittleHelperHelper
{
	...
	public static function getButtons($view_mode)
	{
		...
		$document->addStyleDeclaration('
				...
				// joomla 30
				'
				#littlehelpermoduletoolbar-status button i.icon-out-2:before,
				#littlehelpermoduletoolbar-cpanel button i.icon-out-2:before {content:""}
				#littlehelpermoduletoolbar-status-popup-ccfs button i.icon-out-2 {background-image:url(components/com_littlehelper/assets/images/cachefs16.png);}
				#littlehelpermoduletoolbar-cpanel-popup-ccfs button i.icon-out-2 {background-image:url(components/com_littlehelper/assets/images/cachefs16.png);}
				#littlehelpermoduletoolbar-status-popup-ccjoomla button i.icon-out-2 {background-image:url(components/com_littlehelper/assets/images/cachecc16.png);}
				#littlehelpermoduletoolbar-cpanel-popup-ccjoomla button i.icon-out-2 {background-image:url(components/com_littlehelper/assets/images/cachecc16.png);}
				#littlehelpermoduletoolbar-status-popup-trash button i.icon-out-2 {background-image:url(components/com_littlehelper/assets/images/bin16.png);}
				#littlehelpermoduletoolbar-cpanel-popup-trash button i.icon-out-2 {background-image:url(components/com_littlehelper/assets/images/bin16.png);} 					
		...
		if (empty($params->button_recycle) || (int)($params->button_recycle)==1) {
			$bar->appendButton('Popup', 'trash', $showtext?JText::_("COM_LITTLEHELPER_CMD_RECYCLE_SHORT"):"", 'index.php?option=com_littlehelper&amp;task=trash_n_cache.emptyrecyclebin&amp;tmpl=component&amp;view=modal', $width, $height, $top, $left, $onClose);	
					}
		if (empty($params->button_cache) || (int)($params->button_cache)==1) {

			if (!isset($params->button_cache_usefs) || (int)($params->button_cache_usefs)==1) {
				$bar->appendButton('Popup', 'ccfs', 	  $showtext?JText::_("COM_LITTLEHELPER_CMD_FS_SHORT"):"", 'index.php?option=com_littlehelper&amp;task=trash_n_cache.cleanfscache&amp;tmpl=component&amp;view=modal', $width, $height, $top, $left, $onClose);
			}else{
				$bar->appendButton('Popup', 'ccjoomla', 	  $showtext?JText::_("COM_LITTLEHELPER_CMD_JOOMLA_SHORT"):"", 'index.php?option=com_littlehelper&amp;task=trash_n_cache.cleanjoomlacache&amp;tmpl=component&amp;view=modal', $width, $height, $top, $left, $onClose);
			}
		}		
