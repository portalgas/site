...
class UsersModelUser extends JModelAdmin
{
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		/*
		 * fractis  ctrl l'organization_id
		 */
		$userJoomla = JFactory::getUser();
		
		if(isset($userJoomla->organization['Organization']) && isset($userJoomla->organization['Organization']['id']))
			$organization_id = $userJoomla->organization['Organization']['id'];
		
		// non serve
		if(isset($userJoomla->supplier['Supplier']) && isset($userJoomla->supplier['Supplier']['id']))
			$supplier_id = $userJoomla->supplier['Supplier']['id'];
		
		
		/*
		 * se empty($result->organization_id) sono in nuovo utente
		 */
		if(!isset($result->organization_id) && !empty($result->organization_id) && $organization_id!=$result->organization_id) {
			return null;
		}
