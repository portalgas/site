...
class UsersModelUsers extends JModelList
{
	 ...
	 * fractis filtro in #__users per organization_id
	 *
	...
	protected function getListQuery()
	{
		...
		/*
		 * fractis from #__user_profiles
		 *	 $query->from($db->quoteName('#__user_profiles').' AS profiles ,'.$db->quoteName('#__users').' AS a ');
		 */
		...
		/*
		 * fractis join user + user_profiles.profile.organization_id
		$userJoomla = JFactory::getUser();
		$organization_id = $userJoomla->organization['Organization']['id'];
		$query->where(' (profiles.user_id = a.id AND
				    	 profiles.profile_value = \'"'.$organization_id.'"\' AND 
						 profiles.profile_key = \'profile.organization_id\')');
		*/		
		/*
         * fractis filtro per organization_id
 		 */
		$userJoomla = JFactory::getUser();
		if(isset($userJoomla->organization['Organization']))
			$query->where('a.organization_id = '.(int) $userJoomla->organization['Organization']['id']);
		else
		if(isset($userJoomla->supplier['Supplier']))
			$query->where('a.supplier_id = '.(int) $userJoomla->supplier['Supplier']['id']);		
		else
			$query->where('a.organization_id = 0');
