...
<table class="table table-striped" class="adminlist">
    <thead>
        <tr>
            ...
            <th width="5">
                <?php echo JHtml::_('grid.sort', 'COM_DPCALENDAR_VIEW_EVENTS_MODAL_COLUMN_ID', 'id', $listDirn, $listOrder); ?>
            </th>
        </tr>
    </thead>
    <tfoot>
...
{ ?>
        <tr class="row<?php echo $i % 2; ?>">
            <td>
				<a class="pointer" onclick="if (window.parent) window.parent.<?php echo $this->escape($function) . "('" . $item->id . "', '" . $this->escape(addslashes($item->title)) . "', '" . $this->escape($item->catid) . "', null, '" . $this->escape(DPCalendarHelper::getEventRoute($item->id, $item->catid)); ?>', 'null', null);">
