...
DPCalendarHelper::loadLibrary(array('jquery' => true, 'bootstrap' => true));
...
(function($) {
	...
	Joomla.submitbutton = function(task) {
		if (task == 'translate.update') {
			dpjQuery('#resources tbody .resource').each(function( index ) {
				var el = dpjQuery(this);
				dpjQuery.ajaxQueue({
					type: 'POST',
					url: 'index.php?option=com_dpcalendar&task=translate.update',
					data: {resource: el.attr('id')},
					beforeSend: function () {
						el.prev().find('i').attr('class', 'icon-refresh');
					},
					success: function (data) {
						var json = dpjQuery.parseJSON(data);
						Joomla.renderMessages(json.messages);

						var el = dpjQuery('#' + json.data.resource);
						el.prev().find('i').attr('class', 'icon-ok-circle');
					}
				}, 'update');
			});
		}
			...
			dpjQuery.ajaxQueue({
				...
				success: function (data) {
					...
					for (var i in json.languages) {
						...
						var el = dpjQuery('#' + json.resource + '-' + language.tag).find('span');
