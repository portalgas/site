...
class DPCalendarControllerTranslate extends JControllerLegacy
{
	...
	public function fetch ()
	{
		...
		{
			$resourceData = DPCalendarHelperTransifex::getData('resource/' . $resource . '/stats');
			$transifexLanguages = json_decode($resourceData['data']);
			foreach ($transifexLanguages as $langCode => $tr)
			{
				$code = DPCalendarHelperTransifex::getLangCode($langCode);
	...
	public function update ()
	{
		...
		foreach ((array) json_decode($resourceData['data']) as $langCode => $lang)
		{
			...
			$content = DPCalendarHelperTransifex::getData('resource/' . $resource . '/translation/' . $code . '?file=1');
			...
			if (strpos($resource, 'plg_') === 0)
			{
				...
				{
					$path = JPATH_PLUGINS . '/';
					$path .= $plugin->folder . '/' . $plugin->element . '/language/' . $code . '/' . $code . '.' .
							 str_replace('plg_dpcalendar', 'plg_dpcalendar_dpcalendar', $plugin->name);
				}
			}
		...
		DPCalendarHelper::sendMessage(JText::sprintf('COM_DPCALENDAR_VIEW_TOOLS_TRANSLATE_UPDATE_RESOURCE_SUCCESS', $resource), false,
				array(
						'resource' => $resource
				));
	}
