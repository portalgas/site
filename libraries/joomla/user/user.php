...
class JUser extends JObject
{
	...
	public function __construct($identifier = 0)
	{
		...
		else
		{
			...
			$this->ACLsuppliersIdsOrganization = "";
			$this->ACLsuppliersIdsDes = "";
			$this->org_id = 0;
			$this->supplier_id = 0;
			$this->supplier = array();
		}
	...
	public function save($updateOnly = false)
	{
		...
		try
		{
			 ...
			 * fractis 
			 * 
			 * se BackOffice imposto al nuovo/modificato user l'organization_id dell'amministratore
			 * se FrontEnd   e' gia' passato dal form di registrazione
			*/
			if(!empty($my->id)) {
				$organization = $my->organization;
				$table->organization_id = $organization['Organization']['id'];
				
				if(isset($my->supplier)) {
					$supplier = $my->supplier;
					if(isset($supplier['Supplier']))
						$table->supplier = $supplier['Supplier']['id'];					
				}
			} 
	...
	public function load($id)
	{
		...
		/*
		 * fractis all'object User aggiungo 
		$arrUserProperties = $table->getProperties();
		$db = JFactory::getDbo();
		$db->setQuery(
				'SELECT profile_value FROM #__user_profiles' .
				' WHERE user_id = '.(int) $id." AND profile_key = 'profile.organization_id'"
		);
		$organization_id = $db->loadResult();
		$organization_id = preg_replace('/^(")+/', '', $organization_id);
		$organization_id = preg_replace('/(")+$/', '', $organization_id);
		$arrUserProperties['organization_id'] = $organization_id;
		*/
		$arrUserProperties = $table->getProperties();
		$arrUserProperties['organization'] = array();
		$arrUserProperties['supplier'] = array();
		$arrUserProperties['ACLsuppliersIdsOrganization'] = "";
		$arrUserProperties['org_id'] = "";
		$this->_params->loadString($table->params);

		// Assuming all is well at this point lets bind the data
		// fractis $this->setProperties($table->getProperties());
		$this->setProperties($arrUserProperties);
