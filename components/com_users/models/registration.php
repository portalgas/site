...
class UsersModelRegistration extends JModelForm
{
	...
	public function activate($token)
	{
		...
		if (($userParams->get('useractivation') == 2) && !$user->getParam('activate', 0))
		{
			...
			// get all admin users  fractis
			$query = 'SELECT name, email, sendEmail, id' .
						' FROM #__users' .
						' WHERE sendEmail=1 '.
						' AND (organization_id = '.$user->organization_id.' OR organization_id = 0) ';
	...
	{
		/*
		 * fractis, creo logging
		 */
		jimport('joomla.log.log');
		JLog::addLogger(array('text_file' => 'com_users.log'),JLog::ALL,'com_users');
		
		JLog::add("----- ini registrazione ---- ", JLog::DEBUG);
		...
		foreach ($temp as $k => $v) {
			$data[$k] = $v;
			// fractis, PHP Notice:  Array to string conversion 
			// JLog::add($k.": ".$v, JLog::DEBUG);
		}
		...
		/*
		 * fractis, ctrl se organization_id valorizzato
		 */ 
		if (empty($data['organization_id'])) {
			$this->setError("Devi indicare il G.A.S. di appartenenza");
			return false;
		}		
		...
		*/
		$query = "SELECT j_group_registred FROM k_organizations Organization WHERE id = ".$user->organization_id;
		//echo '<br />'.$query;
		JLog::add("  Query per registrazione Utente nel Gruppo Front-end dell'organizzazione ", JLog::DEBUG);
		JLog::add("     ".$query, JLog::DEBUG);
		$db->setQuery( $query );
		$rows = $db->loadObject();
		if(!empty($rows)) {
			$group_id = $rows->j_group_registred;
			JLog::add("  Organization.j_group_registred ".$group_id, JLog::DEBUG);
			
			$query = $db->getQuery(true);
			$query->clear();
			$query->values($user->id . ',' . $group_id);
			$groups = true;
			$query->insert($db->quoteName('#__user_usergroup_map'));
			$query->columns(array($db->quoteName('user_id'), $db->quoteName('group_id')));
			$db->setQuery($query);
			$db->query();			
			JLog::add("  Inserisco in __user_usergroup_map ".$query, JLog::DEBUG);
		}
		else
			JLog::add("  Organization.j_group_registred non trovato!", JLog::DEBUG);
		...
		$return = JFactory::getMailer()->sendMail($data['mailfrom'], $data['fromname'], $data['email'], $emailSubject, $emailBody);
		if ($return !== true) 
			JLog::add("   invio mail a ".$data['email']." da ".$data['mailfrom']." ".$data['fromname']." - esito NO ", JLog::DEBUG);
		else
			JLog::add("   invio mail a ".$data['email']." da ".$data['mailfrom']." ".$data['fromname']." - esito OK ", JLog::DEBUG);
		...
		if (($params->get('useractivation') < 2) && ($params->get('mail_to_admin') == 1)) {
			...
			$emailBodyAdmin = JText::sprintf(
				...
				$data['name'].' '.$data['username'],  // fractis
				$data['email'],
				$data['siteurl']
			);
			...
			// get all admin users
			/*
			 * fractis invio mail filtrato per organization_id
			 */
			$query = 'SELECT name, email, sendEmail' .
					' FROM #__users' .
					' WHERE sendEmail=1'.
					' AND (organization_id = 0 OR organization_id = '.$user->organization_id.')';
			// echo '<br />'.$query;
			JLog::add("  Query per invio mail amministratori ", JLog::DEBUG);
			JLog::add("     ".$query, JLog::DEBUG);
			$db->setQuery( $query );
			$rows = $db->loadObjectList();
			...
			foreach( $rows as $row )
			{
				...
				// Check for an error.
				if ($return !== true) {
					JLog::add("   Invio mail a ".$row->email.' '.JText::_('COM_USERS_REGISTRATION_ACTIVATION_NOTIFY_SEND_MAIL_FAILED'), JLog::DEBUG);
					$this->setError(JText::_('COM_USERS_REGISTRATION_ACTIVATION_NOTIFY_SEND_MAIL_FAILED'));
					return false;
				}
				else 
					JLog::add("   Invio mail a ".$row->email." OK", JLog::DEBUG);
			}
		}
		// Check for an error.
		if ($return !== true) {
