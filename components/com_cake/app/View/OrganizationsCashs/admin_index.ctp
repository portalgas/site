...
echo $this->Form->create('OrganizationsCash', ['id' => 'formGas']);
...
if(empty($results['User'])) { 
	echo $this->element('boxMsg',array('class_msg' => 'notice resultsNotFound', 'msg' => "Non ci sono gasisti"));
}
else {

	$options = ['options' => $cashLimits, 'value' => $results['OrganizationsCash']['cashLimit'], 'label'=>__('CashLimit'), 'required'=> true];
