				...
				foreach ($order['ExportRows'] as $rows) {
					...
					foreach ($rows as $typeRow => $cols) {
						...
						switch ($typeRow) {
							...
							case 'TRTOT':
							/*
								$html .= '<td colspan="4" style="text-align:right;">Quantit&agrave;&nbsp;totale&nbsp;&nbsp;</td>';
								$html .= '<td style="text-align:center;" width="'.$output->getCELLWIDTH50().'">'.$cols['QTA'].'</td>';
								$html .= '<td style="text-align:right;">&nbsp;'.$cols['IMPORTO'].$this->App->traslateQtaImportoModificati($cols['ISIMPORTOMOD']).'</td>';
								if(($order['Order']['hasTrasport']=='Y' && $order['Order']['trasport']!='0.00') ||
									($order['Order']['hasCostMore']=='Y' && $order['Order']['cost_more']!='0.00') ||
							...
							*/							break;								
							case 'TRDATA':
								...
								if($order['Order']['hasTrasport']=='Y') {
									...
									$html .= '<td style="text-align:center;" width="'.$output->getCELLWIDTH50().'">'.$cols['QTA'].$this->App->traslateQtaImportoModificati($cols['ISQTAMOD']).'</td>';
									$html .= '<td width="'.($output->getCELLWIDTH100()+$output->getCELLWIDTH30()).'" style="text-align:right;">'.$cols['IMPORTO'].$this->App->traslateQtaImportoModificati($cols['ISIMPORTOMOD']).'</td>';
								}
							break;
							case 'TRDATABIS':
								if($order['Order']['hasTrasport']=='Y')									$colspan = '7';								else									$colspan = '5';
...
if(!empty($results['RequestPaymentsStoreroom'])) {
	...
	foreach($results['RequestPaymentsStoreroom'] as $numResult => $result) {
		...
		$html .= '<td width="'.$output->getCELLWIDTH70().'">'.$result['Storeroom']['prezzo_e'].'</td>';
 ...
 *   solo se e' in stato pagato, se no sballa con i calcoli di cassa (importo_pagato == 0 e li mette tutti in cassa)
 */
//if($results['SummaryPayment']['stato'] == 'PAGATO' ) {
	...
	echo '<br >importo_pagato - importo_dovuto = '.$delta_cassa;

	if($results['SummaryPayment']['importo_pagato'] > 0) {
		...
		$importo_finale = number_format($results['SummaryPayment']['importo_pagato'],2,Configure::read('separatoreDecimali'),Configure::read('separatoreMigliaia'));

	}
	else {
		if($delta_cassa < 0) {
			/*
			 *    user ha un CREDITO con la cassa (ha in cassa + di quello richiesto)
			 */
			$delta_cassa = (-1 * $delta_cassa); 
			$delta_cassa = number_format($delta_cassa,2,Configure::read('separatoreDecimali'),Configure::read('separatoreMigliaia'));
		...
		/*	
			...
			$html .= '</tr>';
		*/	
			$results['SummaryPayment']['importo_dovuto'] = 0;
		}
		
		$importo_finale = number_format($results['SummaryPayment']['importo_dovuto'],2,Configure::read('separatoreDecimali'),Configure::read('separatoreMigliaia'));
	}


$importo_finale = $importo_dovuto;
	...
	if($importo_finale!='0,00') {
		...
		*/
		...
		$html .= 'Importo:&nbsp;'.$importo_finale.'&nbsp;&euro;<br />';
