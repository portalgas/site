<?php
$this->App->d([$user->organization['Organization']['id'], $organization_id]);
...
switch($totUserFlagPrivacy) {
	case 0:
		...
		$link = $this->Html->link('Gestisci il ruolo '.__('UserGroupsUserFlagPrivacy'), ['controller' => 'UserGroupMaps', 'action' => 'index', null, 'group_id='.Configure::read('group_id_user_flag_privacy')]);		
	break;
	case 1:
...
if($user->organization['Organization']['id'] == $organization_id)
	echo ' '.$link;
