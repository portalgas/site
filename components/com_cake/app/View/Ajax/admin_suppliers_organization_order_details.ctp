...
if(!empty($results)) {
	if(!empty($results['Supplier']['img1']) && file_exists(Configure::read('App.root').Configure::read('App.img.upload.content').'/'.$results['Supplier']['img1']))
		...
		if(isset($results['Order']['owner_articles'])) {
			echo '<div class="input text ">';
			echo '<label>'.__('organization_owner_articles').'</label> ';						
			switch ($results['SuppliersOrganization']['owner_articles']) {
				case 'PACT':
					echo $this->App->traslateEnum('ProdGasSupplier'.$results['Order']['owner_articles']);
				break;
				case 'SUPPLIER':
					echo $this->App->traslateEnum('ProdGasSupplier'.$results['Order']['owner_articles']);
				break;
            ...
            if($results['Order']['owner_articles']!=$results['SuppliersOrganization']['owner_articles']) {
                echo '<div class="input text ">';
                echo '<label>Il produttore ha la gestione del listino degli articoli configurata al</label> ';
