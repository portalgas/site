...
echo $this->element('search_articles');
...
if(!empty($results)) { 
	...
	echo '<th>'.__('Bio').'</th>';
	echo '<th>'.__('Type').'</th>';
	echo '<th class="text-center">'.__('Stato').'</th>';
	echo '<th class="text-center">Associabili<br/>agli ordini<span style="float:right;">'.$this->App->drawTooltip('Articoli associabili ad un ordine', __('toolFlag_presente_articlesorders'),$type='HELP',$pos='LEFT').'</span>';
...
foreach ($results as $numResults => $result) {
	...
	*/		
	...
	echo '<td style="cursor:pointer;" data-attr-id="'.$result['Article']['id'].'" data-attr-organization-id="'.$result['Article']['organization_id'].'" data-attr-field="stato" class="articleUpdate stato_'.$this->App->traslateEnum($result['Article']['stato']).'" title="'.__('toolTipStato').'" ></td>';
		*/ 
	echo '<td class="text-center articleUpdate stato_'.$this->App->traslateEnum($result['Article']['stato']).'" title="'.__('toolTipStato').'" ></td>';
...
echo '<div class="paging">';
	...
	if($iniCallPage)
		echo $this->element('boxMsg', ['class_msg' => 'success resultsNotFound', 'msg' => __('msg_search_no_parameter')]);
	else
