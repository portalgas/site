...
foreach($results['Delivery'] as $numDelivery => $result['Delivery']) {
		...
		foreach($result['Delivery']['Order'] as $numOrder => $order) {
			...
			if(isset($order['ArticlesOrder'])) { // cosi' escludo gli ordini senza acquisti
				...
				foreach($order['ArticlesOrder'] as $numArticlesOrder => $articlesOrder) {
					...
					$name = $order['Article'][$numArticlesOrder]['name'].' '.$this->App->getArticleConf($order['Article'][$numArticlesOrder]['qta'], $order['Article'][$numArticlesOrder]['um']);
					...
					* */
					...
					if($order['Article'][$numArticlesOrder]['bio'])
						$html .= 'Bio';
					$html .= '</td>';
					...
					$html .= '<td width="'.$output->getCELLWIDTH70().'">'.$this->App->getArticlePrezzoUM($order['ArticlesOrder'][$numArticlesOrder]['prezzo'], $order['Article'][$numArticlesOrder]['qta'], $order['Article'][$numArticlesOrder]['um'], $order['Article'][$numArticlesOrder]['um_riferimento']).'</td>';
					...
					*/
					...
					if(isset($resultsWithModifiesOrder['SummaryOrder'][0])) {
						...
						if($importo > 0) {
							...
							$html .= '	<th colspan="2" style="text-align:right;"></th>';
							$html .= '	<th style="text-align:center;"></th>';
							$html .= '	<th colspan="3" style="text-align:right;">Totale dell\'ordine&nbsp;&nbsp;'.number_format($importo,2,Configure::read('separatoreDecimali'),Configure::read('separatoreMigliaia')).'&nbsp;&euro;</th>';
							$html .= '</tr>';
						}
					}					
				}
	...
	if(isset($storeroomResults['Delivery'][$numDelivery])) {
		...
		if($delivery['totStorerooms']) {
			...
			$html .= '			<th width="'.$output->getCELLWIDTH70().'">'.__('PrezzoUM').'</th>';
			...
			foreach($delivery['Storeroom'] as $numStoreroom  => $storeroom) {
				...
				if($storeroom['SuppliersOrganization']['id']!=$supplier_organization_id_old) {
					...
					$html .= '<td colspan="8" class="trGroup">'.__('Supplier').': '.$storeroom['SuppliersOrganization']['name'];
					if(!empty($storeroom['SuppliersOrganization']['descrizione'])) $html .= '/'.$storeroom['SuppliersOrganization']['descrizione'];
					$html .= '</td>';
					$html .= '</tr>';
				}
				...
				$html .= '	<td width="'.$output->getCELLWIDTH30().'">';
				if($storeroom['Article']['bio']=='Y') $html .= 'Bio';
				$html .= '</td>';
				...
				$html .= '<td width="'.$output->getCELLWIDTH70().'">'.$this->App->getArticlePrezzoUM($storeroom['prezzo'], $storeroom['Article']['qta'], $storeroom['Article']['um'], $storeroom['Article']['um_riferimento']).'</td>';
