<?php
/**
 * override perche' si perdeva sort / direction
 *
 ...
 */
	protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		if ($this->{$check}($model)) {
			...
			$url = array_merge(array('sort' => $sort, 'direction' => $direction), $url, array('order' => null));	
	...
	public function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = $start; $i < $params['page']; $i++) {
				...
				$out .= $this->Html->tag($tag, $this->link($i, $url, $options), compact('class')) . $separator;
			}
