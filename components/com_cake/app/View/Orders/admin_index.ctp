...
if(!empty($results)) {
	...
	foreach ($results as $i => $result) {
		...
		if($delivery_id_old==0 || $delivery_id_old!=$result['Delivery']['id']) {
			...
			if(!empty($tot_importo_to_delivery)) {
				$tot_importo_to_delivery = number_format($tot_importo_to_delivery,2,Configure::read('separatoreDecimali'),Configure::read('separatoreMigliaia'));
				echo '<tr><td></td><td></td><td></td><td colspan="'.($colspan-3).'">Totale della consegna '.$tot_importo_to_delivery.'&nbsp;&euro;</td></tr>';
				$tot_importo_to_delivery = 0;
			}
		...
		*/
		...
		$tot_importo_to_delivery += $result['Order']['tot_importo'];
