...
echo '<div class="tab-pane fade active in" id="tabs-0">';
	 ...
	 * consegna
	 */
	echo $this->element('boxOrdersPromotionDelivery', ['modalita' => 'ADD']);
	echo $this->Html->div('clearfix','');
	
	echo $this->App->drawDate('ProdGasPromotionsOrganizationsManager', 'data_inizio', __('DataInizio'), $this->request->data['ProdGasPromotionsOrganizationsManager']['data_inizio_db']);
	
	echo $this->App->drawDate('ProdGasPromotionsOrganizationsManager', 'data_fine', __('DataFine'), $this->request->data['ProdGasPromotionsOrganizationsManager']['data_fine_db']);
...
$(document).ready(function() {
	...
	$("#modalOrderMailMsg").on("shown.bs.modal", function () {
		...
		var testo = "";
		var prodGasPromotionsOrganizationsManagerDataInizioDb = $('#ProdGasPromotionsOrganizationsManagerDataInizioDb').val();
		if(prodGasPromotionsOrganizationsManagerDataInizioDb=='' || prodGasPromotionsOrganizationsManagerDataInizioDb==undefined) 
				testo = "Devi indicare la data di apertura dell'ordine";
		else {
				var resultCompare = compare_date_today(prodGasPromotionsOrganizationsManagerDataInizioDb);
