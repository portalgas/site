...
echo '<fieldset>';
	...
	echo $this->Form->input('contact_name', ['value' => $contact_name, 'label' => __('Name'), 'required' => false]);
	echo $this->Form->input('contact_mail', ['value' => $contact_mail, 'label' => __('Email'), 'required' => false]);
	echo $this->Form->input('contact_phone', ['value' => $contact_phone, 'label' => __('Telephone'), 'required' => false]);
		...
		foreach ($organizationResults as $numResult => $result):
			...
			*/
			...
			if(isset($result['Delivery']) && !empty($result['Delivery']))
				echo $this->Form->input('trasport', ['id' => 'trasport-'.$result['Organization']['id'], 'name' => 'data[ProdGasPromotion][Organization]['.$result['Organization']['id'].'][trasport]', 'label' => false, 'type' => 'text','style' => 'display:inline', 'default' => '0,00','tabindex'=>($i+1),'class' => 'double', 'after' => '&nbsp;&euro;', 'required'=>'false']);
			echo '</td>';	
			echo '<td style="white-space: nowrap;">';
			if(isset($result['Delivery']) && !empty($result['Delivery']))
				echo $this->Form->input('costMore', ['id' => 'costMore-'.$result['Organization']['id'], 'name' => 'data[ProdGasPromotion][Organization]['.$result['Organization']['id'].'][costMore]', 'label' => false, 'type' => 'text','style' => 'display:inline', 'default' => '0,00','tabindex'=>($i+1),'class' => 'double', 'after' => '&nbsp;&euro;', 'required'=>'false']);
			echo '</td>';
