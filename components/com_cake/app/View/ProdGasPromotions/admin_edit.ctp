<?php
/*
echo "<pre>DATI PROMOZIONE \n";
print_r($this->request->data);
echo "</pre>";
echo "<pre>Dati ARTICOLI \n";
print_r($prodGasArticleResults);
echo "</pre>";
echo "<pre>Dati ORGANIZATIONS \n";
...
echo '<div id="tabs-0">';
	...
	echo $this->Form->input('data_inizio',array('type' => 'text','size'=>'30','label' => __('Data inizio'), 'value' => $prod_gas_promotion_data_inizio, 'required'=>'false'));
	...
	echo $this->Form->input('data_fine',array('type' => 'text','size'=>'30','label' => __('Data fine'), 'value' => $prod_gas_promotion_data_fine, 'required'=>'false'));
	...
	if(isset($file1)) {
		...
		echo '<img src="'.Configure::read('App.server').Configure::read('App.web.img.upload.prod_gas_promotions').'/'.$this->request->data['ProdGasPromotion']['supplier_id'].'/'.$file1->name.'" />';	
...
echo '<div id="tabs-1">';
	...
	foreach ($prodGasArticleResults as $numResult => $result):
		...
		$importo_originale = ($result['ProdGasArticlesPromotion']['qta'] * $result['ProdGasArticle']['prezzo']);
		...
		echo $this->Form->input('qta_in_promozione',array('id' => 'qta_in_promozione-'.$result['ProdGasArticle']['id'], 'name' => 'data[ProdGasPromotion][ProdGasArticlesPromotion]['.$result['ProdGasArticle']['id'].'][qta]', 
														'value' => $result['ProdGasArticlesPromotion']['qta'],
														 'label' => false, 'type' => 'text','size' => '3','default' => '0','tabindex'=>($i+1),'class' => 'qta_in_promozione noWidth', 'required'=>'false'));
		echo '</td>';
		echo '<td>';
		...
		echo '</td>';
		echo '<td>';
		echo '<input type="hidden" class="importo_originale" id="importo_originale-'.$result['ProdGasArticle']['id'].'" value="'.$importo_originale.'" name="data[ProdGasPromotion][ProdGasArticlesPromotion]['.$result['ProdGasArticle']['id'].'][importo_originale]" />';
		echo '<span id="importo_originale_label-'.$result['ProdGasArticle']['id'].'">'.$importo_originale_.'</span><span> &euro;</span>';
		echo '</td>';
		echo '<td>';
		echo $this->Form->input('importo_scontato',array('id' => 'importo_scontato-'.$result['ProdGasArticle']['id'], 'name' => 'data[ProdGasPromotion][ProdGasArticlesPromotion]['.$result['ProdGasArticle']['id'].'][importo_scontato]', 
														 'value' => $result['ProdGasArticlesPromotion']['importo_'],
														 'label' => false, 'type' => 'text','size' => '5','default' => '0,00','tabindex'=>($i+1), 'class' => 'importo_scontato noWidth double', 'after' => '€', 'required'=>'false'));
		echo '</td>';			
		echo '<td>';
...
echo '<div id="tabs-2">';
	...
	echo '<table cellpadding="0" cellspacing="0">';
		...
		foreach ($organizationResults as $numResult => $result):
			...
			echo '<td>';
			echo $this->Form->input('trasport',array('id' => 'trasport-'.$result['Organization']['id'], 'name' => 'data[ProdGasPromotion][Organization]['.$result['Organization']['id'].'][trasport]', 'label' => false, 'type' => 'text','size' => '6','default' => $trasport,'tabindex'=>($i+1),'class' => 'noWidth double', 'after' => '€', 'required'=>'false'));
			echo '</td>';
			echo '<td>';
			echo $this->Form->input('costMore',array('id' => 'costMore-'.$result['Organization']['id'], 'name' => 'data[ProdGasPromotion][Organization]['.$result['Organization']['id'].'][costMore]', 'label' => false, 'type' => 'text','size' => '6','default' => $costMore,'tabindex'=>($i+1),'class' => 'noWidth double', 'after' => '€', 'required'=>'false'));
			echo '</td>';
