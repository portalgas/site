...
<h2 class="ico-edit-cart">
	<?php echo __('Add ArticlesOrder').' ('.count($results).')';?>
...
?>
	<div class="panel-group">
	  <div class="panel panel-primary">
		...
		<div id="collapse1" class="panel-collapse collapse <?php if(count($results)>0) echo 'in';?>">
	  ...
	  <div class="panel panel-primary">
		...
		<div id="collapse2" class="panel-collapse collapse <?php if(count($articles)>0) echo 'in';?>">
		  <div class="panel-body">
			<?php 
			if(count($articles)>0) {
			?>
			...
			<tr>
				...
				foreach ($articles as $ii => $article):
					...
					echo '</td>';
					echo '<td nowrap>'.$article['Article']['prezzo_e'].'</td>';
					echo '<td style="text-align:center;">'.$article['Article']['pezzi_confezione'].'</td>';
					echo '<td style="text-align:center;">'.$article['Article']['qta_minima'].'</td>';
					echo '<td style="text-align:center;">'.$article['Article']['qta_massima'].'</td>';
					echo '<td style="text-align:center;">'.$article['Article']['qta_multipli'].'</td>';
					echo '<td style="text-align:center;">'.$article['Article']['qta_minima_order'].'</td>';
					echo '<td style="text-align:center;">'.$article['Article']['qta_massima_order'].'</td>';
...
echo '&nbsp;';
echo '<div><a class="btn btn-primary" href="index.php?option=com_cake&amp;controller=Orders&amp;action=home&amp;id='.$order['Order']['id'].'" title="'.__('OrderCreatedGoToHome').'">'.__('OrderCreatedGoToHome').'</a></div>';
...
$(document).ready(function() {
	...
	$('#formGas').submit(function() {
		...
		for(ii = 0; ii < $("input[name='article_id_selected']:checked").length; ii++) {
			article_id_selected += $("input[name='article_id_selected']:checked").eq(ii).val()+',';
			
			article_id = $("input[name='article_id_selected']:checked").eq(ii).val();
		}
		...
		if(i==$("input[name='article_order_key_selected']").length && ii==0) {
			alert("Scegli gli articoli da associare all'ordine");
			return false;
		}
