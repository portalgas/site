...
$this->Html->addCrumb(__('ProdGasArticlesSyncronizesAdvanced'));
...
echo __('ProdGasArticlesSyncronizesAdvanced');
echo '<div class="actions-img">';
echo '<ul>';
echo '<li>';
echo $this->Html->link(__('ProdGasArticlesSyncronizesSimple'), array('action' => 'index', null, 'organization_id='.$organization_id.'&type=SIMPLE'), array('class' => 'action actionBackup','title' => __('ProdGasArticlesSyncronizesSimple')));
echo '</li>';
echo '</ul>';
echo '</div>';
echo '</h2>';
...
echo '<fieldset>';
		...
		<tr>
			...
			<th class="title" colspan="5"><?php echo __('ProdGasSupplierArticlesOrganization');?></th>
		...
		foreach ($articlesResults as $numResult => $result):
			...
			if($prodGasArticleExist) {
				...
				if($isArticleInCart) 
					echo $this->App->drawTooltip(__('ProdGasArticle'),__('toolTipProdGasArticleSyncroNotExist'),$type='INFO');
				echo '</td>';
			}
			...
			$differente = 'green'; 
			if($prodGasArticleExist) {
				if($result['ProdGasArticle']['name']!=$result['Article']['name'] || 
				   $result['ProdGasArticle']['prezzo']!=$result['Article']['prezzo'] || 
				   $result['ProdGasArticle']['qta']!=$result['Article']['qta'] || 
				   $result['ProdGasArticle']['um']!=$result['Article']['um'] || 
				   $result['ProdGasArticle']['pezzi_confezione']!=$result['Article']['pezzi_confezione'] || 
				   $result['ProdGasArticle']['qta_minima']!=$result['Article']['qta_minima'] || 
				   $result['ProdGasArticle']['qta_multipli']!=$result['Article']['qta_multipli']
				  )
				  $differente = 'red';
			} 
			else
				$differente = 'yellow';
			echo '<td style="';
			echo 'background-color:'.$differente.';';
			echo '"></td>';
			...
			if($prodGasArticleExist) {
				echo $this->Html->link(null, array('action' => 'syncronize_flag_presente_articlesorders', null, 'organization_id='.$organization_id.'&article_id='.$result['Article']['id'].'&type=ADVANCED'), array('class' => 'action actionOnOff', 'title' => __('ProdGasSyncronizeFlagPresenteArticlesordersButton')));
				echo $this->Html->link(null, array('action' => 'syncronize_update', null, 'organization_id='.$organization_id.'&prod_gas_article_id='.$result['ProdGasArticle']['id']), array('class' => 'action actionSyncronize actionGetCategory','id' => $result['ProdGasArticle']['id'], 'title' => __('ProdGasSyncronizeUpdate')));			
			}
			else {
				if(!$isArticleInCart)
					echo $this->Html->link(null, array('action' => 'syncronize_delete', null, 'organization_id='.$organization_id.'&article_id='.$result['Article']['id'].'&type=ADVANCED'), array('class' => 'action actionDelete', 'title' => __('ProdGasSyncronizeDelete')));
				else
					echo $this->Html->link(null, array('action' => 'syncronize_flag_presente_articlesorders', null, 'organization_id='.$organization_id.'&article_id='.$result['Article']['id'].'&type=ADVANCED'), array('class' => 'action actionOnOff', 'title' => __('ProdGasSyncronizeFlagPresenteArticlesordersButton')));
			}
			echo '</td>';
	...
	echo '<div class="panel-heading">';
    echo '<h4 class="panel-title">';
	echo '<a data-toggle="collapse" data-parent="#accordion" href="#collapse3"><i class="fa fa-lg fa-plus" aria-hidden="true"></i> Articoli del GAS ancora da associare ('.count($prodGasArticlesAllResults).')</a>';
	echo '</h4>';
	echo '</div>';
	...
	} // end if(count($articlesToAssociateListResults)>0) 
