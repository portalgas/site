...
class PactSupplierImportsController extends AppController {
	...
	public function admin_index() { 
		...
		/*
		 * sql per settare la gestione del listino articoli al produttore
		 */
		...
		$results = $this->PactSupplierImport->getPactSuppliers($this->user, $organization_id, 0, [], $debug);		
	...
	public function admin_organizations_articles($supplier_id=0) { 
		...
		$results = $this->PactSupplierImport->getOrganizationsArticles($this->user, $supplier_id, $debug);
	...
	public function admin_updateField($supplier_id, $organization_id, $id) {
		...
		self::d($sql, false);
		try {
			$results = $this->PactSupplierImport->query($sql);
			$esito = 'OK';
		}
		catch (Exception $e) {
		...
		if(!empty($prod_gas_supplier_id)) {
			...
			*/
			...
			$options['conditions'] = ['Article.organization_id' =>  $this->user->organization['Organization']['id'],
										   ...
										   // escludo quelli gia' associati al PactSupplier
