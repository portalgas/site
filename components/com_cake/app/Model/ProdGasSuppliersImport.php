...
class ProdGasSuppliersImport extends AppModel {
	...
	/*
	 * estrae i dati di un GAS legato al produttore (ProdGasSuppliers)
	 */
	public function getGas($user, $organization_id, $debug=false) {
		
		$results = [];
		
		$prodGasSuppliersResults =  $this->getProdGasSuppliers($user, $user->organization['Organization']['id'], $organization_id, [], $debug); 
		if(!empty($prodGasSuppliersResults)) {
			$results = current($prodGasSuppliersResults['Supplier']['Organization']);
		}
	 ...
	 */
	public function getProdGasSuppliers($user, $prod_gas_organization_id=0, $gas_organization_id=0, $filters=[], $debug=false) {
		...
		foreach($organizationResults as $numResult => $organizationResult) {
			...
			$prodGasSupplierResults = $ProdGasSupplier->getOrganizationSupplier($user, $organization_id, $filters, false);	
			if(!empty($prodGasSupplierResults)) {
				...
				if(!empty($gas_organization_id)) {
					foreach($prodGasSupplierResults['Organization'] as $numResult2 => $prodGasSupplierResult) {
						
						self::d($prodGasSupplierResult['Organization']['id'].' '.$gas_organization_id,$debug);
						
						if($prodGasSupplierResult['Organization']['id'] != $gas_organization_id)
							unset($prodGasSupplierResults['Organization'][$numResult2]);
					}
									
				} // if(!empty($gas_organization_id))
			
				$organizationResults[$numResult]['Supplier'] = $prodGasSupplierResults;
			}
	...
	public function getOrganizationsArticles($user, $supplier_id, $debug=false) {
		...
		*/
		 ...
		 */
		$prodGasSuppliersResults = $this->getProdGasSuppliers($user, $supplier_id, $debug);
		/*
		echo "<pre>";
		print_r($prodGasSuppliersResults);
		echo "</pre>";
		*/
		
		$organizations_owner_articles = [];
		$suppliers_organization_ids = [];	
		foreach($prodGasSuppliersResults as $prodGasSuppliersResult) {
			foreach($prodGasSuppliersResult['SuppliersOrganization'] as $suppliersOrganization) {
