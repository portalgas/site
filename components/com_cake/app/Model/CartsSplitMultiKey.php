...
App::uses('AppModel', 'Model');
/* * override perche' $primaryKeyArray = array('organization_id', 'order_id', 'article_id', 'user_id', 'num_split');*/class CartsSplitMultiKey extends AppModel {
	
	public $name = 'CartsSplit';
		
	var $primaryKeyArray = array('organization_id', 'order_id', 'article_organization_id', 'article_id', 'user_id', 'num_split');	
	/*
	...
	*/		public function exists($organization_id=0, $order_id=0, $article_organization_id=0, $article_id=0, $user_id=0, $num_split=0) {
		if(empty($organization_id) || empty($order_id) || empty($article_organization_id) || empty($article_id) || empty($user_id) || empty($num_split)) {			return false;		}
		...
		return (bool)$this->find('count', array(				'conditions' => $conditions,				'recursive' => -1,				'callbacks' => false		));	}		public function read($organization_id=0, $order_id=0, $article_organization_id=0, $article_id=0, $user_id=0, $num_split=0, $fields = NULL, $id = NULL) {					$this->validationErrors = array();
		$conditions = array(				$this->alias . '.organization_id' => $organization_id,				$this->alias . '.order_id' => $order_id,
				$this->alias . '.article_organization_id' => $article_organization_id,
				$this->alias . '.article_id' => $article_id,				$this->alias . '.user_id' => $user_id,
						$this->alias . '.num_split' => $num_split		);
		...
		if(isset($this->data[$this->alias]['num_split'])) 				 $num_split = $this->data[$this->alias]['num_split'];					if(!$this->exists($organization_id, $order_id, $article_organization_id, $article_id, $user_id, $num_split))			$created = true;		else			$created = false;

		if($created) {			/*			 * insert			*/			$sql = "INSERT INTO					".Configure::read('DB.prefix')."carts_splits					(organization_id, order_id, article_organization_id, article_id, user_id, num_split, importo_forzato, created)";
			$sql .= "					VALUES (					".$organization_id.",					".$order_id.",
	...
	/*	 * se user_id cancello tutti gli acquisti di un ordine	*/	public function delete($organization_id=0, $order_id=0, $article_organization_id=0, $article_id=0, $user_id=0, $num_split=0) {					$sql = "DELETE					FROM						".Configure::read('DB.prefix')."carts_splits	   				WHERE	   					organization_id = ".(int)$organization_id."
