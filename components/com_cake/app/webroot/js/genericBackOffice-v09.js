...
My.prototype = {
    ...
    bindEvents: function () {
		...
		$("#scroll_to_top a").css({	'display' : 'none', 'z-index' : '9', 'position' : 'fixed', 'top' : '100%', 'width' : '110px', 'margin-top' : '-30px', 'right' : '50%', 'margin-left' : '-50px', 'height' : '20px', 'padding' : '3px 5px', 'font-size' : '14px', 'text-align' : 'center', 'color' : '#FFFFFF', 'background-color' : '#625043', '-moz-border-radius' : '5px', '-khtml-border-radius' : '5px', '-webkit-border-radius' : '5px', 'opacity' : '.8', 'text-decoration' : 'none'});
...
function setExportDocsParameters(doc_options) {
    ...
    if (doc_options == 'to-users-label') {
        a = $("input[name='user_phone3']:checked").val();
        b = $("input[name='user_email3']:checked").val();
        c = $("input[name='user_address3']:checked").val();
        if ($("input[name='trasportAndCost3']").length > 0)
            d = $("input[name='trasportAndCost3']:checked").val();
        else
            d = 'N';
        e = $("input[name='user_avatar3']:checked").val();
        f = $("input[name='delete_to_referent3']:checked").val();
        if ($("input[name='codice3']").length > 0)
            g = $("input[name='codice3']:checked").val();
        else 
        	g = 'N';                
    } 
    else
    if (doc_options == 'to-users-articles-label') {
        a = $("input[name='user_phone30']:checked").val();
        b = $("input[name='user_email30']:checked").val();
        c = $("input[name='user_address30']:checked").val();
        if ($("input[name='trasportAndCost30']").length > 0)
            d = $("input[name='trasportAndCost30']:checked").val();
        else
            d = 'N';
        e = $("input[name='user_avatar30']:checked").val();
        f = $("input[name='delete_to_referent30']:checked").val();
        if ($("input[name='codice30']").length > 0)
            g = $("input[name='codice30']:checked").val();
        else 
        	g = 'N';                
    }     
    else
	if (doc_options == 'to-articles-monitoring') {
        if ($("input[name='colli1']").length > 0)
            a = $("input[name='colli1']:checked").val();
        else
            a = 'N';
    } else    
    ...
    if (doc_options == 'to-users-schema') {
        a = $("input[name='user_phone_schema']:checked").val();
        b = $("input[name='user_email_schema']:checked").val();
        c = $("input[name='user_address_schema']:checked").val();
        d = $("input[name='user_avatar_schema']:checked").val();
        e = 'N';
        f = 'N'; 
        g = 'N'; 
    } else
