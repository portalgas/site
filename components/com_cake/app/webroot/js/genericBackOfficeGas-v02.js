...
function viewTesoriereRequestPaymentSottoMenuBootstrap(request_payment_id) {
	if(request_payment_id==null)  return;
		
	var idSelector = ''; 
	if ($('#order-sotto-menu-'+request_payment_id).length==0)
		idSelector = '.order-sotto-menu-unique';
	else
		idSelector = '#order-sotto-menu-'+request_payment_id;
	
	$.ajax({
		type: "get", 
		url: "/administrator/index.php?option=com_cake&controller=Tesoriere&action=sotto_menu_tesoriere_request_payment_bootstrap&request_payment_id="+request_payment_id+"&format=notmpl",
		data: "",
		success: function(response) {
			$(idSelector).html(response);
		},
...
/* non + utilizzata
function viewReferenteTesoriereRequestPaymentSottoMenuBootstrap(request_payment_id) {
	if(request_payment_id==null)  return;
	...
	$.ajax({
		type: "get", 
		url: "/administrator/index.php?option=com_cake&controller=Tesoriere&action=sotto_menu_referentetesoriere_request_payment&request_payment_id="+request_payment_id+"&format=notmpl",
		data: "",
		success: function(response) {
			$(idSelector).css('background', 'none repeat scroll 0 0 transparent');
			$(idSelector).html(response);
