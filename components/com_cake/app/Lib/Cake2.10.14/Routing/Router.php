 ...
 */
	...
	public static function url($url = null, $full = false) {
		...
		if($request->url=='administrator/index.php')
			$result = Configure::read('App.server').'/administrator/index.php?option=com_cake';
		else
			$result = Configure::read('App.server').'?option=com_cake';
		
		/*
		 *  $url = parametro passato html->link('label', array('controller', 'action', 'id', 'key=value'))
		...
		if (!is_array($url)) {
			...
			*/
			 ...
			 * controller
			 */
			$result .= '&controller='.ucfirst($request->params['controller']);
			...
			if(isset($request->params['prefix']) && 
			  strncmp($action, $request->params['prefix'], strlen($request->params['prefix']))===0)  // elimina prefix admin_ dall'action
				$action = substr($action, strlen($request->params['prefix'])+1);
				
			$result .= '&action='.$action;	

			/*
			 * moduli in POST: se c'e' ID glielo metto nell'url
			 */
			if(isset($request['pass']['id'])) 
				$result .= '&id='.$request['pass']['id'];
		}
		else {	
			...
			/*
			 * pulisco url da eventuali se non devo gestire la paginazione
			 *  [sort:bio] => 
    		 *  [direction:asc]
			 */
			if(!key_exists('page',$url))
				foreach($url as $key => $value) 
					if(strpos($key,':')!==false) unset($url[$key]);
				
			/*
			 * controller
			 */
			if(isset($url['controller'])) {
				$result .= '&controller='.ucfirst($url['controller']);  
				unset($url['controller']);  // elimino perche' alla fine faccio il ciclo degli eventuali (id, key=value) 
			}	
			else
				$result .= '&controller='.ucfirst($request->params['controller']);
			...
			if(strncmp($action, $request->params['prefix'], strlen($request->params['prefix']))===0) // elimina prefix admin_ dall'action
				$action = substr($action, strlen($request->params['prefix'])+1);
			 ...
			 * 			null, 'supplier_id=65', 'fractis=33')
			 * 
			 * [controller] => articles
 			 * [action] => index
    		 * [0] => 
    		 ...
    		 * [page] => 2
    		 * 
    		 *  ciclo degli eventuali (id, key=value)  			
			 */
			$key_con_due_punti = array('page','sort','direction');
			foreach ($url as $key =>$value) {
				...
				if(is_numeric($key)) {
					if($key=='0') { 
						...
						// non lo considero se l'ultimo carattere e' : (sort:) oppure = (FilterArticleName=)
						if(substr($value,strlen($value)-1) != ':' && substr($value,strlen($value)-1) != '=')
							$result .= '&'.$value;     // supplier_id=9
					}
				}
				else {
					if(in_array($key, $key_con_due_punti)) { // page
						if(!empty($value))
							$result .= '&'.$key.':'.$value;
					}
					else
						$result .= '&'.$key.'='.$value;    // supplier_id
				} // foreach ($url as $key =>$value)
			}
			
			if(isset($request['params']['named'])) {
				foreach ($request['params']['named'] as $key => $value) {
					if(!empty($value))
						$result .= '&'.$key.':'.$value;
				}
			}
